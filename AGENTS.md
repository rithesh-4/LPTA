...
- **Keep in sync:** compare contract in `src/main.cpp` `--compare` ↔ `serve_dashboard.py` `compare_histories`: score components/verdict/coverage, presence-vs-effect maps, target states, finding shapes/messages, one-decimal pcts, canonical `--json` shape. JSON keys (`regression_score`, `verdict`, `score_components`, `coverage`, `compat`) are API — don't rename. Gate: `tests/compare_golden.sh` (12 fixtures, byte-equivalent normalized output) must pass before merging any compare change.
